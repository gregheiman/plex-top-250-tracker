...
/**
 * Class to fetch the needed info to run the program from the user, either
...
public class FetchPlexInfo {
    ...
    private PlexInfoFetchingMethod fetchMethod;

    public enum PlexInfoFetchingMethod {
        AUTOMATIC, MANUAL
    }

    public FetchPlexInfo(PlexInfoFetchingMethod fetchMethod) {
    ...
    public void fetchPlexInfo() {
        switch (fetchMethod) {
        case AUTOMATIC:
            automaticallyFetchPlexInfo();
            break;
        case MANUAL:
            manuallyFetchPlexInfo();
            break;
        }
    }

    /**
     * If the user prefers to enter in all of the needed information manually or the
     * automatic system doesn't work
     */
    ...
    public void automaticallyFetchPlexInfo() {
        ...
        while (true) {
            ...
            try {
                ...
                grabPlexAuthToken = Jsoup.connect("https://plex.tv/users/sign_in.xml?X-Plex-Client-Identifier=1")
                        .data("user[login]", plexLogin).data("user[password]", plexPassword).followRedirects(false)
        ...
        // User selects which server they want to use and which library in that server
        // they want to use
        while (true) {
            ...
            try {
                // Grab the connection tag from the unfiltered xml as this contains the IP
                // address and port number
                connections = connectToServerList().getElementsByTag("Connection");
            } catch (IOException e) {
                System.out.println(
                        "There was an IO error fetching the list of Plex servers using the information provided");
            } catch (Exception e) {
            ...
            try {
                ...
                System.out
                        .println("There was an error fetching the IP address of the chosen server. Please select a new"
                                + " server.");
            }
    ...
    /**
     * User selects which server they would like to use from the list provided by
     * the Plex API
     *
     * @param listOfPlexServers - the unfiltered xml from hitting the Plex API for
     *                          server devices
     ...
     * 
     * @param connections - Filtered part of the xml that contains the IP address
     ...
     * 
     * @param connections - Filtered part of the xml that contains the port number
     *                    for the selected server
     * @param serverNum   - The number of the selected server in the list
    ...
    /**
     * User connects to the newly selected server and fetches a list of the
     * libraries that server contains
     * 
     * @return - The document containing unfiltered XML of all the libraries inside
     *         the server
    ...
    /**
     * User selects which library they would like to use from a list provided by the
     * XML from connectToSelectedServer()
     * 
     * @param listOfPlexLibraries - Unfiltered XML containing information about the
     *                            libraries inside of the server
